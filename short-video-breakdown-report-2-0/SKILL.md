...
4. **Write the single-video diagnosis**
   ...
   - Keep the 3-second hook diagnosis from 1.0, but add `voice/timbre emotion`, `TA filter`, and `search/conversion promise`.
   - Add a compact `星推搜直复盘` card or table.
   - In the timeline, include rows for content function, spoken point, visual action, proof, search cue, conversion cue, strength, and risk when relevant.
...
4. **字幕、音频与画面校准**: transcript, OCR, vocal emotion, BGM, cut-point, and proof moments.
5. **高光截图**: each screenshot must prove a content function.
6. **时间轴拆解**: real beats only, with content function and marketing handoff.
